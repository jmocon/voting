...
require_once ("../App_Code/Property.php");
require_once ("../App_Code/PropertyModel.php");
...
<html class="no-js css-menubar" lang="en">
	<head>
		...
		<title>Inquiries | <?php echo $clsContent->GetName(); ?> - Member</title>
							...
							<div class="panel">
								...
								<div class="panel-body">
									<table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
										...
										<tbody>
											<?php
											$lstMessages = $clsMessages->GetByUserId($_SESSION['uid']);
											foreach($lstMessages as $mdlMessages)
											{
											?>
											<tr id="trMessage<?php echo $mdlMessages->getId(); ?>">
												<td><?php echo $mdlMessages->getProperty_Id(); ?></td>
												<td><?php echo $mdlMessages->getSubject(); ?></td>
												<td><?php echo $mdlMessages->getEmail(); ?></td>
												<td><?php echo $mdlMessages->getDateCreated(); ?></td>
												<td>
													<a href="JavaScript:void(0);" id="Read<?php echo $mdlMessages->getId(); ?>" class="btn btn-icon btn-default btn-pure btn-xs <?php echo ($mdlMessages->getRead() == 0) ? '':"d-none";?>" data-toggle="tooltip" data-original-title="Mark as Read" onclick="read(<?php echo $mdlMessages->getId(); ?>);">
														<i class="icon icon-xs wb-check m-0" aria-hidden="true"></i>
													</a>
													<a href="JavaScript:void(0);" id="UnRead<?php echo $mdlMessages->getId(); ?>" class="btn btn-icon btn-default btn-pure btn-xs <?php echo ($mdlMessages->getRead() == 0) ? "d-none":'';?>" data-toggle="tooltip" data-original-title="Mark as Unread" onclick="unread(<?php echo $mdlMessages->getId(); ?>);">
														<i class="icon icon-xs wb-close m-0" aria-hidden="true"></i>
													</a>
													
													<span data-toggle="modal" data-target="#ModalWrapper" onclick="displayProperty(<?php echo $mdlMessages->getId(); ?>);">
													...
													<span data-toggle="modal" data-target="#ModalWrapper" onclick="deleteShow(<?php echo $mdlMessages->getId(); ?>);">
